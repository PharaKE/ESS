...
function closeMobileSidebar() {
  ...
  document.getElementById('historyPanel').classList.remove('open');  // keep this here only
  document.getElementById('sidebarOverlay').classList.remove('active');
...
function newChat() {
  ...
  item.innerHTML = `<div class="ch-dot green"></div><div class="ch-body"><div class="ch-title">New Conversation</div><div class="ch-time">Just now</div></div>`;
  item.onclick = () => selectChat(item, -1);
...
function appendBotMsg(text) {
  ...
  row.innerHTML = `
    <div class="msg-avatar">
      <img src="images/KEbot.png" alt="KE" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">
      <div class="msg-avatar-fb"></div>
    </div>
    <div>
      <div class="msg-bubble">${md2html(text)}</div>
      <div class="msg-time">${now}</div>
    </div>`;
  msgs.appendChild(row);
  scrollBottom();
